 ...
 * to HTML at build time with markdown-it, so the app never needs a runtime
 * markdown renderer. Relationships (post <-> author <-> tag) are left as id
...
const md = new MarkdownIt({
  html: true, // content already contains raw HTML (e.g. <br>)
  linkify: true,
  typographer: false,
});
