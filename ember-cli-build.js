...
function treeForPublic(app) {
  const config = app.project.config(process.env.EMBER_ENV || 'development');

  const blogConfig = config.blog || {};

  let appPrefix = join(app.project.configPath(), '../..');

// TODO: once this is stable and added to documentation then we  should enable this warning
//     if(_.isNil(blogConfig.paginate)) {
//       app.ui.writeWarnLine(`You have not set paginate to 'true' or 'false' in your blog config. In the next major version of empress-blog this will default to true
//
...
//     }
  ...
  if(!existsSync(tagFolder)) {
    app.ui.writeWarnLine(`As of empress-blog@1.7 you must define your tags in the same way as you define your authors. We will auto generate tag files for you but this behaviour will be removed in empress-blog@2.0.
...
Please generate tags using 'ember generate tag your-tag-name'`);
  ...
  if (blogConfig.host) {
    ...
    if(app.ui) {
      app.ui.writeWarnLine(`Host is not configured so no RSS feed will be generated
